...
if(!function_exists("almas_gold_customer_deposit_list")) {
    function almas_gold_customer_deposit_list()
    {
        ...
        $table_name_orders_deposit = $wpdb->prefix . "almas_gold_deposit";
        $deposit_orders = $wpdb->get_results($wpdb->prepare("SELECT * \r\n                    FROM " . $table_name_orders_deposit . " \r\n                    WHERE user_id = %d \r\n                    AND transaction_processed = %d \r\n                    ORDER BY id DESC", $user_id, 1));
        if($deposit_orders) {
            require_once "view/customer-deposit-list-html.php";
        } else {
            echo "\r\n                <div style=\"padding: 10px;\">\r\n                    <h4>هیچ سفارشی وجود ندارد!</h4>\r\n                </div>\r\n                ";
        }
    }
}
almas_gold_customer_deposit_list();
