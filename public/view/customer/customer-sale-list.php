...
if(!function_exists("almas_gold_customer_sale_list")) {
    function almas_gold_customer_sale_list()
    {
        ...
        $table_name_orders_sale = $wpdb->prefix . "almas_gold_sale";
        $sale_data = $wpdb->get_results($wpdb->prepare("SELECT * \r\n                    FROM " . $table_name_orders_sale . " \r\n                    WHERE user_id = %d \r\n                    AND transaction_processed = %d \r\n                    ORDER BY id DESC", $user_id, 1));
        if($sale_data) {
            require_once "view/customer-sale-list-html.php";
        } else {
            echo "\r\n                <div style=\"padding: 10px;\">\r\n                    <h4>هیچ سفارشی وجود ندارد!</h4>\r\n                </div>\r\n                ";
        }
    }
}
almas_gold_customer_sale_list();
